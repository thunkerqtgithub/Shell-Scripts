...
# runs every 0 minute hour

RSYNC="/opt/homebrew/bin/rsync -a --delete"
TARGET="/Volumes/Hourly/Incrementals"
OLD="--link-dest=$TARGET/hourly.1"
SOURCE="/Volumes/Hourly/Incrementals/Source/"
...
deleteOldestSnapshot() {
	if [ -d "$TARGET/hourly.0" ]; then
		rm -fr "$TARGET/hourly.11" >/dev/null 2>&1
	fi
}
...
rotateSnapshot() {
	for i in {11..1}
	do
   		mv "$TARGET/hourly.$[${i}-1]" "$TARGET/hourly.${i}" >/dev/null 2>&1
		sync
	done
}

createSnapshot() {
	$RSYNC "$OLD" "$SOURCE" "$TARGET/hourly.0"
}

mount | grep -i "/Volumes/Hourly"
if [ $? -eq 0 ]; then
