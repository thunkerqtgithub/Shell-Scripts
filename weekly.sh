...
# runs every Sunday @8:45am

RSYNC="/opt/homebrew/bin/rsync -a --delete"
TARGET="/Volumes/Weekly/Incrementals"
OLD="--link-dest=$TARGET/weekly.1"
SOURCE="/Volumes/Weekly/Incrementals/Source/"
...
deleteOldestSnapshot() {
	if [ -d "$TARGET/weekly.0" ]; then
		rm -fr "$TARGET/weekly.3" >/dev/null 2>&1
	fi
}
...
rotateSnapshot() {
	for i in {3..1}
	do
   		mv "$TARGET/weekly.$[${i}-1]" "$TARGET/weekly.${i}" >/dev/null 2>&1
		sync
	done
}

createSnapshot() {
	$RSYNC "$OLD" "$SOURCE" "$TARGET/weekly.0"
}

mount | grep -i "/Volumes/Weekly"
if [ $? -eq 0 ]; then
